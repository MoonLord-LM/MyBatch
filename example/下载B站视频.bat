...
REM 支持输入视频链接单个下载，或者输入包含视频链接的 txt 文件路径批量下载

REM 依赖的软件如下
REM https://github.com/yt-dlp/yt-dlp
REM https://github.com/FFmpeg/FFmpeg
REM https://github.com/denoland/deno

REM 当前文件夹下必须有 www.bilibili.com_cookies.txt 文件用于登录
REM 获取 Cookie 的插件如下
REM https://chromewebstore.google.com/detail/get-cookiestxt-locally/cclelndahbckbenkjhflpdbgdldlbecc

REM 从 UP 主的主页视频界面，获取所有的视频链接
REM 打开浏览器控制台，搜索并复制 search 请求，Copy as fetch
REM 然后在后面拼接以下代码并执行：.then(response => response.json()).then(data => { if (data && data.data && data.data.list && data.data.list.vlist) { const videoList = data.data.list.vlist; let allLinks = ''; videoList.forEach(video => { allLinks += `https://www.bilibili.com/video/${video.bvid}/\n`; }); console.log(allLinks); } else { console.error('未能找到有效的视频列表数据'); } }).catch(error => console.error('Error:', error));

REM 支持这种链接：https://www.bilibili.com/festival/2021bnj?bvid=BV12o4y1o7Sb
...
:download_video
    ...
    REM festival 活动页链接（同时含 /festival/ 和 ?bvid=）直接原样传入 yt-dlp，不做处理
    set "skip_strip=0"
