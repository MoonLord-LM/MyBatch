...
:loop
    ...
    if "!input!"=="" (
        ...
        for %%f in (*.mkv) do (
            set "file_name=%%f"
            echo 开始处理 !file_name!
            if not exist "!file_name!.mp4" (
                ffmpeg.exe -i "!file_name!" -c copy -map_metadata 0 -movflags +faststart "!file_name!.new.mp4"
            ) else (
                ffmpeg.exe -i "!file_name!" -c copy -map_metadata 0 -movflags +faststart "!file_name!.mp4"
            )
            echo.
        )
        goto loop
        ...
        if not exist "!input!.mp4" (
            ...
            ffmpeg.exe -i "!input!" -c copy -map_metadata 0 -movflags +faststart "!input!.mp4"
        )
    )
    echo.
