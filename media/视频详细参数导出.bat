...
if "!param1!" == "" (
    ...
    if exist "!param1!\" (
        ...
        echo 开始处理文件："!param1!"
        set "json_file=!param1_dir!!param1_name!.json"
        if exist "!json_file!" (
            echo 已存在："!json_file!"，跳过此文件
        ) else (
            "!ffprobe_path!" -v error -show_streams -show_format -print_format json "!param1!" > "!json_file!"
            if !errorlevel! neq 0 (
...
if not "!working_dir!" == "" (
    ...
    for /f "delims=" %%f in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; Get-ChildItem -LiteralPath $env:working_dir -File -Force -Recurse | Where-Object { $_.Extension -match $env:ext_filter } | ForEach-Object { $_.FullName }"') do (
        setlocal disabledelayedexpansion
        set "video_file=%%f"
        set "file_dir=%%~dpf"
        set "base_name=%%~nf"
