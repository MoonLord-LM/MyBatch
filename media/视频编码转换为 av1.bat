...
powershell -NoProfile -Command "Write-Host '视频编码转换为 av1 格式（高质量 -crf 24 -preset 2）' -ForegroundColor Green"
        ...
        if "!is_av1!"=="1" (
            ...
            if exist "!output_file!" (
                ...
                if "!need_convert!"=="1" (
                    ...
                    "!ffmpeg_path!" -i "!param1!" -c:v libsvtav1 -crf 24 -preset 2 -c:a flac -compression_level 8 "!output_file!"
                    if !errorlevel! neq 0 (
                    ...
                    "!ffmpeg_path!" -i "!param1!" -c:v libsvtav1 -crf 24 -preset 2 -c:a copy "!output_file!"
                    if !errorlevel! neq 0 (
                        if exist "!output_file!" ( del /f /q "!output_file!" )
                        echo 转换失败
                    ) else (
    ...
    for /f "delims=" %%f in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; Get-ChildItem -LiteralPath $env:file_path -File -Force -Recurse | Where-Object { $_.Extension -match $env:ext_filter } | ForEach-Object { $_.FullName }"') do (
        ...
        if "!is_av1!"=="1" (
            ...
            if exist "!output_file!" (
                ...
                if "!need_convert!"=="1" (
                    echo 检测到不支持的音频编码：!audio_codec!，正在转换为 FLAC 格式...
                    "!ffmpeg_path!" -i "!video_file!" -c:v libsvtav1 -crf 24 -preset 2 -c:a flac -compression_level 8 "!output_file!"
                    if !errorlevel! neq 0 (
                    ...
                    "!ffmpeg_path!" -i "!video_file!" -c:v libsvtav1 -crf 24 -preset 2 -c:a copy "!output_file!"
                    if !errorlevel! neq 0 (
                        echo set /a "convert_failed+=1">> "!temp_set!"
                        if exist "!output_file!" ( del /f /q "!output_file!" )
                        echo 转换失败
