...
powershell -NoProfile -Command "Write-Host '将带 EXIF 拍摄时间的图片，重命名为 IMG_YYYYMMDD_HHMMSS_fff 格式，默认使用系统时区' -ForegroundColor Green"
...
powershell -NoProfile -Command "Write-Host '没有 EXIF 拍摄时间的图片，不做处理' -ForegroundColor Green"
echo.
...
if not "!working_dir!" == "" (
    ...
    set /a "fail_total=no_exif+name_conflict+rename_failed"
    echo 共计：!total! 个，成功：!ok_total! 个，失败：!fail_total! 个 & REM
    echo 其中，重命名成功 !succeeded! 个，已符合规范 !already_ok! 个，已存在同名文件 !name_conflict! 个，无 EXIF 时间跳过 !no_exif! 个，重命名失败 !rename_failed! 个
)
