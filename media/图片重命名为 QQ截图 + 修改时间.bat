...
if "%~1" == "" (
    ...
    for /f "delims=" %%f in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; Get-ChildItem -LiteralPath $env:file_path -File -Force -Recurse | Where-Object { $_.Extension -match $env:ext_filter } | ForEach-Object { $_.FullName }"') do (
        ...
        if /i "!base_name:~0,10!"=="Screenshot" (
            ...
            set "formatted_time="
            for /f "delims=" %%t in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; (Get-Item -LiteralPath $env:img_file).LastWriteTime.ToString('yyyyMMddHHmmss')" 2^>nul') do (
    ...
    if exist "!img_file!\" (
        ...
        set "ext_ok="
        for /f "delims=" %%e in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; if ($env:file_ext -match '^\.(jpg|png|bmp|gif)$'){Write-Output 'ok'}" 2^>nul') do set "ext_ok=%%e"
        if not "!ext_ok!"=="ok" (
