...
if "!param1!" == "" (
    ...
    if exist "!param1!\" (
        ...
        if "!has_cover!"=="0" (
            ...
            "!ffmpeg_path!" -y -i "!param1!" -map 0:a? -map 0:v? -map 0:s? -map -0:v:disp:attached_pic -c copy "!temp_video_file!"
            if !errorlevel! neq 0 (
...
if not "!working_dir!" == "" (
    ...
    for /f "delims=" %%f in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; Get-ChildItem -LiteralPath $env:file_path -File -Force -Recurse | Where-Object { $_.Extension -match $env:ext_filter } | ForEach-Object { $_.FullName }"') do (
        ...
        if "!has_cover!"=="0" (
            ...
            "!ffmpeg_path!" -y -i "!video_file!" -map 0:a? -map 0:v? -map 0:s? -map -0:v:disp:attached_pic -c copy "!temp_video_file!"
            if !errorlevel! neq 0 (
