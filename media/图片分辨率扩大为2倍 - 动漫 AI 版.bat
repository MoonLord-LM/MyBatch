...
powershell -NoProfile -Command "Write-Host '图片分辨率扩大为 2 倍（动漫 AI 版，使用 RealESRGAN realesr-animevideov3 模型）' -ForegroundColor Green"
...
if "!param1!" == "" (
    ...
    if exist "!param1!\" (
        ...
        if exist "!output_file!" (
            ...
            "!video2x_path!" -i "!param1!" -o "!temp_video!" -s 2 -p "realesrgan" --realesrgan-model "realesr-animevideov3" -c libx264rgb -e crf=0 -e preset=ultrafast
            if exist "!temp_video!" (
...
if not "!working_dir!" == "" (
    ...
    for /f "delims=" %%f in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; Get-ChildItem -LiteralPath $env:file_path -File -Force -Recurse | Where-Object { $_.Extension -match $env:ext_filter } | ForEach-Object { $_.FullName }"') do (
        ...
        if exist "!output_file!" (
            ...
            echo 正在扩大为："!output_file!"
            set "temp_video=%temp%\MyBatch_%random%_%random%_%random%_%random%.tmp.mp4"
            "!video2x_path!" -i "!image_file!" -o "!temp_video!" -s 2 -p "realesrgan" --realesrgan-model "realesr-animevideov3" -c libx264rgb -e crf=0 -e preset=ultrafast
            if exist "!temp_video!" (
                "!ffmpeg_path!" -y -i "!temp_video!" -frames:v 1 "!output_file!"
                if !errorlevel! equ 0 (
