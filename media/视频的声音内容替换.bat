...
powershell -NoProfile -Command "Write-Host '保留第 1 个文件的封面、画面、字幕和元数据信息，将第 2 个文件的声音内容，替换到第 1 个文件中，生成新文件' -ForegroundColor Green"
...
for %%i in ("!video1!") do (
    ...
    :: 小写 v 匹配所有视频流（含封面），此处保留第 1 个文件的全部画面和封面
    "!ffmpeg_path!" -y -i "!video1!" -i "!video2!" -map 1:a -map 0:v? -map 0:s? -map_metadata 0 -c copy "!temp_file!"
    if !errorlevel! neq 0 (
