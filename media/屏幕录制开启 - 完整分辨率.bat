...
powershell -NoProfile -Command "Write-Host '双击运行，开始屏幕录制，以完整的分辨率 + 最高 30 的帧率，录制为无损的 mp4 文件' -ForegroundColor Green"
...
goto :after_powershell_block
    ...
    $psi.Arguments = '-y -f gdigrab -framerate 30 -draw_mouse 1 -i desktop -c:v libx264rgb -preset ultrafast -crf 0 -pix_fmt bgr0 -movflags +faststart -metadata creation_time="' + $creation_time + '" ' + $env:output_file;
