...
powershell -NoProfile -Command "Write-Host '双击运行，开始屏幕延时录制，以缩小为 0.5 倍的分辨率，每 3 秒抓取 1 帧，录制为 60 帧/秒播放的 mp4 文件' -ForegroundColor Green"
...
set "output_file=!script_dir!延时录屏_!time_stamp!.mp4"
...
goto :after_powershell_block
    ...
    $psi.Arguments = '-y -f gdigrab -framerate 1/3 -draw_mouse 1 -i desktop -vf scale=iw/2:ih/2,settb=AVTB,setpts=N/(60*TB),fps=60 -c:v libx264rgb -crf 0 -preset ultrafast -pix_fmt bgr0 -movflags +faststart -metadata creation_time="' + $creation_time + '" ' + $env:output_file;
