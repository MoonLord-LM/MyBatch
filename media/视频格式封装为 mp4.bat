...
powershell -NoProfile -Command "Write-Host '双击运行时，自动递归扫描和处理当前目录下所有非 mp4 格式的视频文件' -ForegroundColor Green"
...
if "%~1" == "" (
    ...
    if exist "!video_file!\" (
        ...
        set /a "skipped=0"
        set /a "failed=0"
        for /r "!video_file!" %%f in (*.mkv *.ts *.avi *.wmv *.flv *.rmvb *.rm *.vob *.mpg *.mpeg *.3gp *.m4v *.f4v *.mov *.webm) do (
            setlocal disabledelayedexpansion
            set "video_file=%%f"
            set "file_dir=%%~dpf"
            set "base_name=%%~nf"
