...
powershell -NoProfile -Command "Write-Host '将图片按文件最后修改时间，重命名为 Screenshot_YYYYMMDD_HHMMSS 格式，默认使用系统时区' -ForegroundColor Green"
...
powershell -NoProfile -Command "Write-Host '以 IMG、QQ截图、mmexport 开头的图片，不做处理' -ForegroundColor Green"
echo.
...
if "!param1!" == "" (
    ...
    if exist "!param1!\" (
        ...
        if /i "!base_name:~0,4!"=="IMG_" (
            ...
            set "formatted_time="
            for /f "delims=" %%t in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; (Get-Item -LiteralPath $env:img_file).LastWriteTime.ToString('yyyyMMdd_HHmmss')" 2^>nul') do (
