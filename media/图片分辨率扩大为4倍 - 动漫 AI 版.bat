...
powershell -NoProfile -Command "Write-Host '图片分辨率扩大为 4 倍（动漫 AI 版，使用 RealESRGAN realesr-animevideov3 模型）' -ForegroundColor Green"
...
powershell -NoProfile -Command "Write-Host '需要硬件设备有支持 Vulkan 的显卡' -ForegroundColor Green"
echo.
...
REM 检查 video2x 组件
if exist "!script_dir!Video2X\video2x.exe" (
    set "video2x_path=!script_dir!Video2X\video2x.exe"
) else if exist "!cd!\Video2X\video2x.exe" (
    set "video2x_path=!cd!\Video2X\video2x.exe"
) else if exist "!script_dir!..\Video2X\video2x.exe" (
    set "video2x_path=!script_dir!..\Video2X\video2x.exe"
) else if exist "..\Video2X\video2x.exe" (
    set "video2x_path=..\Video2X\video2x.exe"
) else (
    set "video2x_path=video2x"
)
"!video2x_path!" --version >nul 2>&1
if !errorlevel! neq 0 (
    echo 错误：缺少 video2x 组件
    echo 请从 https://github.com/k4yt3x/video2x/releases 下载，然后放到脚本所在文件夹
    "explorer.exe" "https://github.com/k4yt3x/video2x/releases"
    echo.
    pause
    endlocal & endlocal & exit /b 1
)
...
if "!param1!" == "" (
    ...
    if exist "!param1!\" (
        ...
        set "output_file=!file_dir!!base_name!_4x!file_ext!"
        if exist "!output_file!" (
            echo 已存在："!output_file!"，跳过
        ) else (
            echo 正在扩大为："!output_file!"
            set "temp_video=%temp%\MyBatch_%random%_%random%_%random%_%random%.tmp.mp4"
            "!video2x_path!" -i "!param1!" -o "!temp_video!" -s 4 -p "realesrgan" --realesrgan-model "realesr-animevideov3" -c libx264rgb -e crf=0 -e preset=ultrafast
            if exist "!temp_video!" (
            ...
            if exist "!temp_video!" ( del /f /q "!temp_video!" )
        )
    )
)
...
if not "!working_dir!" == "" (
    ...
    for /f "delims=" %%f in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; Get-ChildItem -LiteralPath $env:file_path -File -Force -Recurse | Where-Object { $_.Extension -match $env:ext_filter } | ForEach-Object { $_.FullName }"') do (
        ...
        if exist "!output_file!" (
            ...
            echo 正在扩大为："!output_file!"
            set "temp_video=%temp%\MyBatch_%random%_%random%_%random%_%random%.tmp.mp4"
            "!video2x_path!" -i "!image_file!" -o "!temp_video!" -s 4 -p "realesrgan" --realesrgan-model "realesr-animevideov3" -c libx264rgb -e crf=0 -e preset=ultrafast
            if exist "!temp_video!" (
                "!ffmpeg_path!" -y -i "!temp_video!" -frames:v 1 "!output_file!"
                if !errorlevel! equ 0 (
