...
powershell -NoProfile -Command "Write-Host '将图片的文件修改时间，刷新为文件名中的时间，使二者一致，默认使用系统时区' -ForegroundColor Green"
...
powershell -NoProfile -Command "Write-Host '修改记录和操作结果记录到脚本所在文件夹的 .log 文件中' -ForegroundColor Green"
...
if not "!working_dir!" == "" (
    ...
    echo 批量处理完成
    if not "!succeeded!"=="0" (
        echo 修改明细已记录到日志："!log_file!"
    )
    echo 共计：!total! 个，成功：!succeeded! 个，已一致 !already_ok! 个，非标准命名跳过 !not_standard! 个，未识别到时间 !no_time! 个，刷新失败 !set_failed! 个
)
