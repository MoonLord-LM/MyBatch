...
powershell -NoProfile -Command "Write-Host '视频帧率扩大为 2 倍（通用 AI 版，使用 RIFE rife-v4.26 模型）' -ForegroundColor Green"
...
if "!param1!" == "" (
    ...
    if exist "!param1!\" (
        ...
        set "output_file=!file_dir!!base_name!_2x帧率!file_ext!"
        if exist "!output_file!" (
            echo 已存在："!output_file!"，跳过
        ) else (
            echo 正在扩大为："!output_file!"
            "!video2x_path!" -i "!param1!" -o "!output_file!" -m 2 -p "rife" --rife-model "rife-v4.26" -t 20 -e crf=0 -e preset=ultrafast
            if !errorlevel! equ 0 (
                echo 扩大成功
...
if not "!working_dir!" == "" (
    ...
    for /f "delims=" %%f in ('powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; Get-ChildItem -LiteralPath $env:file_path -File -Force -Recurse | Where-Object { $_.Extension -match $env:ext_filter } | ForEach-Object { $_.FullName }"') do (
        ...
        if exist "!output_file!" (
            ...
            echo 正在扩大为："!output_file!"
            "!video2x_path!" -i "!video_file!" -o "!output_file!" -m 2 -p "rife" --rife-model "rife-v4.26" -t 20 -e crf=0 -e preset=ultrafast
            if !errorlevel! equ 0 (
