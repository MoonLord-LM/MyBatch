...
echo 视频格式封装为 mkv
echo.
echo 双击运行时，自动递归扫描和处理当前目录下所有非 mkv 格式的视频文件
echo.
echo 拖拽单个视频文件到此脚本上时，则只处理该文件
echo.
echo 支持的格式为 mkv ts avi wmv flv rmvb rm vob mpg mpeg 3gp m4v f4v mov webm
echo.
...
if "%~1" == "" (
    ...
    for /r %%f in (*.mp4 *.ts *.avi *.wmv *.flv *.rmvb *.rm *.vob *.mpg *.mpeg *.3gp *.m4v *.f4v *.mov *.webm) do (
        ...
        if exist "!file_dir!!output_file!" (
            ...
            ffmpeg -i "!file_dir!!video_file!" -c copy "!file_dir!!output_file!" -hide_banner -loglevel error
            if errorlevel 1 (
