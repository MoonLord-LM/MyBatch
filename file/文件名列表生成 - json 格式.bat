...
powershell -NoProfile -Command "Write-Host '递归扫描文件夹中的所有文件，生成 json 文件名列表文件' -ForegroundColor Green"
...
powershell -NoProfile -Command "Write-Host '列表文件的内容为：[ 开头 + 每个文件一行的文件对象 + ] 结尾，对象字段为 name、size、modifiedTime，按文件名排序' -ForegroundColor Green"
echo.
...
if not "!working_dir!" == "" (
    ...
    for %%i in ("!file_path!") do (
        ...
        set "output_file=!file_path!\!file_name!.name.json"
        ...
        powershell -NoProfile -Command ^
            ...
            "$items = @($files | Sort-Object Name | ForEach-Object { '    {{\"name\":\"{0}\",\"size\":{1},\"modifiedTime\":\"{2:yyyy-MM-dd HH:mm:ss}\"}}' -f $_.Name, $_.Length, $_.LastWriteTime });" ^
