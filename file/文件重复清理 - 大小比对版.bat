...
powershell -NoProfile -Command "[Console]::OutputEncoding=[Text.Encoding]::UTF8; Add-Type -AssemblyName Microsoft.VisualBasic; ^
    $p1=$env:path1; $p2=$env:path2; ^
    $files1=@(Get-ChildItem -LiteralPath $p1 -File -Recurse); $total=$files1.Count; $deleted=0; $failed=0; ^
    if ($p1 -eq $p2) { ^
        $g=@{}; ^
        foreach ($f in $files1) { $k=$f.Name+'|'+$f.Length; if (-not $g.ContainsKey($k)) { $g[$k]=@() }; $g[$k]+=$f.FullName } ^
        foreach ($k in $g.Keys) { if ($g[$k].Count -ge 2) { $g[$k] | Select-Object -Skip 1 | ForEach-Object { ^
            Write-Host ('删除: '+$_); ^
            try { [Microsoft.VisualBasic.FileIO.FileSystem]::DeleteFile($_,'OnlyErrorDialogs','SendToRecycleBin'); $deleted++ } catch { $failed++ } ^
        } } } ^
    } else { ^
        $m=@{}; ^
        Get-ChildItem -LiteralPath $p2 -File -Recurse | ForEach-Object { $m[$_.Name+'|'+$_.Length]=$true } ^
        foreach ($f in $files1) { $k=$f.Name+'|'+$f.Length; if ($m.ContainsKey($k)) { ^
