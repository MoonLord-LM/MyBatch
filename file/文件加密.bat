...
powershell -NoProfile -Command "Write-Host '双击运行时，按提示输入要加密的文件的路径；也可以拖拽单个文件到此脚本上' -ForegroundColor Green"
...
powershell -NoProfile -Command ^
    ...
    "Write-Host '正在加密，请稍候...';" ^
    "$resultCode = [AesGcmCli]::EncryptFile($env:input_file, $env:output_file, $passwordBytes, [ref]$message);" ^
    "if ($resultCode -ne 0) {" ^
