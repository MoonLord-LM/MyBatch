...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "$rc = [AesGcmCli]::DecryptFile($env:param1_path, $env:output_file, $pwd, [ref]$m);" ^
    "if ($rc -ne 0) { Write-Host ('解密失败：' + $m) -ForegroundColor Red; if (Test-Path $env:output_file) { Remove-Item $env:output_file -Force -ErrorAction SilentlyContinue }; exit 1 };" ^
