...
powershell -NoProfile -Command "Write-Host '递归扫描文件夹中的所有文件，生成 txt 文件名列表文件' -ForegroundColor Green"
...
powershell -NoProfile -Command "Write-Host '列表文件的内容为：文件名 + 字节数 + 修改时间，每个文件一行，按文件名排序' -ForegroundColor Green"
echo.
...
if not "!working_dir!" == "" (
    ...
    for %%i in ("!file_path!") do (
        ...
        set "output_file=!file_path!\!file_name!.name.txt"
        ...
        powershell -NoProfile -Command ^
            ...
            "$lines = @($files | Sort-Object Name | ForEach-Object { '\"{0}\",\"{1}\",\"{2:yyyy-MM-dd HH:mm:ss}\"' -f $_.Name, $_.Length, $_.LastWriteTime });" ^
